	...
	while (entrada != "fin") {
		if (busquedaBinaria(tabla, entrada, pos)) {
			...
			for (int i = 0; i < tabla.elementos[pos].valor.tam; i++) { //Metemos las posiciones en totales de los archivos que queremos mostrar en orden
				buscar(totales, tabla.elementos[pos].valor.elementos[i], aux);
				insertar(orden, aux);
			}
			ordenar(orden);
			...
			delete[] orden.elementos;
			orden.elementos = nullptr;
		}
...
int main(){
	...
	if (crearTabla(tabla, totales, L)){
		...
		delete[] totales.elementos;
		totales.elementos = nullptr;
		for (int i = 0; i < tabla.tam; i++) {
			delete[] tabla.elementos[i].valor.elementos;
			tabla.elementos[i].valor.elementos = nullptr;
		}
		delete[] tabla.elementos;
		tabla.elementos = nullptr;
	}
